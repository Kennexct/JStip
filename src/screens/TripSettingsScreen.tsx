  ...
  useEffect(() => {
    async function loadSettings() {
      try {
        ...
        if (loaded) {
          if (loaded.trip) {
            setOrigin(loaded.trip.origin || 'Seoul');
            setDestination(loaded.trip.destination || 'Jakarta');
            setLimit((loaded.trip.weightLimit || 15).toString());
          }
          if (loaded.currency) {
            setSettings({
              code: loaded.currency.code || 'SGD',
              symbol: loaded.currency.symbol || 'S$',
              manualRate: loaded.currency.manualRate || 13500,
              realtimeRate: loaded.currency.realtimeRate || 13050,
              updatedAt: loaded.currency.updatedAt || new Date().toISOString()
            });
            setPayoutCurrency(loaded.currency.payout || 'IDR');
          }
        }
      } catch (e) {
